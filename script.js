...
// menu searching and filtering the list of the json object according to the category.

    function myFunction(){
        let filter = document.getElementById('myInput').value.toUpperCase();
        let td = document.getElementsByTagName('li');
        //alert(filter);

        for(var i=0; i< json.length ;i++){
            if(json[i].category.toUpperCase().indexOf(filter) == 0)
            {
                td[i].style.display = "";
            }
            else{
...
// end of the menu searching

// display the list of the title of the json object on left side of the body

  function myProduct(){
    document.getElementById("my_added_cart").style.display="none";
    document.getElementById("body_cat").style.display="block";
    document.getElementById("check").style.display="none";

    var list= "<ul>";
    for(let i in json)
    {
        list+="<li id="+i+" onclick=my_click("+i+")>"+json[i].title.slice(0,22)+"</li>"
        document.getElementById("categories").innerHTML=list;
    }
    list= "</ul>";
  }

//   end of the left body function

// to show the selected item (image , price , description ,etc) in a single div of right body
  ...
  function my_click(i)
    ...
    for(let x=0;x<json.length;x++){
        if(x == id)
        {
            ...
            let container_d=document.createElement("div");
            ...
            let price=json[id].price;
            let description=json[id].description;
            dataImage.src=image;
            dataImage.setAttribute("class","product_image");
            dataImage.setAttribute('alt',"image");
            container.append(dataImage);

            container_d.append("Description : "+description.slice(0,100));
            container_p.append("Price :"+" ₹ "+price);
           
            btn.setAttribute("type","text");
            btn.setAttribute("value","1");
            btn.setAttribute("id","name_"+x);
            btn.setAttribute("onkeypress","return IsNumeric(event)");
            btn.setAttribute("ondrop","return false");
            btn.setAttribute("onpaste","return false");
            btn.setAttribute("placeholder","Select The Quantity");
            btn.setAttribute("class","btn");
            container_a.append(btn);
            
            add_button.setAttribute("type","button");
            add_button.setAttribute("value","Add to Cart");
            add_button.setAttribute("class","add_but");
            add_button.setAttribute("onclick","my_cart("+id+")");
            addToCart.appendChild(add_button);

            container_p.setAttribute("class","center");
            container_d.setAttribute("class","center_p");

            container.append(container_p);
            container.append(container_a);
            container.append(container_d);
            container.append(addToCart);
            container.setAttribute("class","product_div");
            document.getElementById("all_prop").appendChild(container);
        }
        else{
            console.log("no cart Clicked")
        }
    }
    }
...
// push performance (index is push in an array to show the myCart operation).

   function my_cart(id)
   {
    let count=document.getElementById("name_"+id).value;
       let cart=id;
       array.push(cart);
       array_Quantity.push(count);
       console.log(cart);
...
// to display the My_cart item

   function show()
   {
       document.getElementById("my_added_cart").style.display="block";
       document.getElementById("check").style.display="block";
       document.getElementById("body_cat").style.display="none";
       ...
       // let total=count+value;
        ...
        for(let i in array)
        {
            count=array_Quantity[i];
            value=array[i];
            total+=json[value].price*count;
            console.log(value);
            list+="<img src="+json[value].image+" alt='image' class='cart_img'>";
            list+="<li>"+"<b>"+"Category :"+"</b>"+json[value].category+"</li>";
            list+="<li>"+"<b>"+"Price :"+"</b>"+json[value].price+"</li>";
            list+="<li>"+"<b>"+"Title :"+"</b>"+json[value].title+"</li>";
            list+="<li>"+"<b>"+"Quantity :"+"</b>"+count+"</li>";
            list+="<li>"+"<b>"+"Grand Total :"+"</b>"+json[value].price*count+"</li>";
            document.getElementById("my_added_cart").innerHTML=list;
        }
        list= "</div>";
   }

//    end of display My_cart

// final result of the shopping its bill and closing.
   function result()
   {
    document.getElementById("my_added_cart").style.display="none";
...
// validation for the only number for the quantity selection.
   ...
   var specialKeys = new Array();
        ...
        function IsNumeric(e) {
            ...
            ret ? '' : alert("number only");
            return ret;
        }

// end of the validation for number only